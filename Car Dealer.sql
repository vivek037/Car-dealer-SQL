...
select CONCAT(Car_Dealer_First_Name,' ',Car_Dealer_Last_Name),Sum(Deposit_paid_for_booking) from Sales_Team JOIN sales_data on (Emp_number = Customer_manager_ID ) group by Emp_number;

select CONCAT(Car_Dealer_First_Name,' ',Car_Dealer_Last_Name) as FULL_EMP_NAME, 
Sum(Deposit_paid_for_booking) as TOTAL_DEPOSITS from Sales_Team JOIN sales_data on (Emp_Number = Customer_manager_ID ) group by Emp_Number;

#question 1
select 
CONCAT(Car_Dealer_First_Name,' ',Car_Dealer_Last_Name) as FULL_EMP_NAME, 
Sum(Car_Price) as TOTAL_SALES 
from Sales_Team JOIN sales_data on (Emp_Number = Customer_manager_ID ) JOIN Cars_Data on ( Customer_Car_Code = Car_Code )
group by Emp_Number;
...
#question 6
select CONCAT(Car_Dealer_First_Name,' ',Car_Dealer_Last_Name) as FULL_EMP_NAME, MIN(Deposit_paid_for_booking) as MINIMUM_DEPOSITS from Sales_Team JOIN sales_data on (Emp_number = Customer_manager_ID ) group by Emp_number;
...
from Sales_Team JOIN sales_data on (Emp_number = Customer_manager_ID ) JOIN Cars_Data on ( Customer_Car_Code = Car_Code ) where Emp_number = '12134'
group by Car_Code;